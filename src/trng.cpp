...
static void health_check_bit(uint8_t bit)
{
    ...
    {
        if ((millis() - health_init_ms) < TRNG_HEALTH_WARMUP_MS)
            return;
...
void trng_process()
{
    ...
    while (ts_read != ts_write)
    {
        ...
        if (dt < TRNG_MIN_INTERVAL_US)
            continue; // keep prev_ts unchanged
...
{
    if (bits_accumulated < TRNG_POOL_BITS)
        return false; // Not enough entropy
    ...
    while (pos < len)
    {
        ...
        bits_accumulated = (bits_accumulated > TRNG_POOL_BITS)
                               ? bits_accumulated - TRNG_POOL_BITS
                               : 0;
    }
