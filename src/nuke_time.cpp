...
void nt_init() {
    ...
    kf_offset = 0.0f;
    kf_drift = 0.0f;
    kf_P[0][0] = 100.0f; kf_P[0][1] = 0.0f;
    kf_P[1][0] = 0.0f;   kf_P[1][1] = 1.0f;
...
static void log_event(const char* msg) {
    ...
    if (event_count < NT_EVENT_LOG_SIZE) event_count++;
...
bool nt_get_event(uint8_t idx, char* buf, size_t len) {
    if (idx >= event_count) return false;
...
static void kalman_predict(float dt) {
    ...
    kf_P[0][0] = p00; kf_P[0][1] = p01;
    kf_P[1][0] = p10; kf_P[1][1] = p11;
}
...
static void discipline_rtc(float drift_ppm_val) {
    ...
    if (fabsf(drift_ppm_val) < 1.0f) return;
...
static void compute_trust() {
    ...
    if (!starved && !source_anomaly_flag) score += 30;
    else if (!starved) score += 15;

    // Calibrated (20 points)
    if (calibrated) score += 20;
    else if (calibrating && cp_count > 0) score += 10;

    // Chi-square passing (20 points) — check last checkpoint
    if (cp_count > 0) {
        const nt_checkpoint_t* last = nt_get_checkpoint(cp_count - 1);
        if (last && last->chi2_pass) score += 20;
        else score += 5; // At least we have data
    }

    // RTC-nuclear agreement (20 points) — based on drift magnitude
    if (calibrated) {
        float abs_drift = fabsf(drift_ppm);
        if (abs_drift < 2.0f) score += 20;
        else if (abs_drift < 10.0f) score += 15;
        else if (abs_drift < 50.0f) score += 10;
        else score += 5;
    }

    // Uptime bonus (10 points) — more checkpoints = more confidence
    if (cp_count >= 24) score += 10;
    else if (cp_count >= 6) score += 7;
    else if (cp_count >= 1) score += 3;
...
static void recalculate_lambda() {
    if (cp_count < NT_MIN_CALIBRATION_H) return;
...
void nt_update(uint32_t rtc_epoch, uint32_t total_counts, float temp_c) {
    ...
    if (elapsed >= NT_CHECKPOINT_SEC) {
        ...
        if (cp_count < NT_CHECKPOINT_COUNT) cp_count++;
...
nt_time_t nt_get_time() {
    ...
    if (!calibrated || lambda_cps <= 0.0f || last_cp_epoch == 0) return t;
...
nt_stratum_t nt_get_stratum() {
    if (!calibrated && !calibrating) return NT_STRATUM_UNCAL;
    if (calibrating)                 return NT_STRATUM_CAL;
    if (source_anomaly_flag)         return NT_STRATUM_HOLDOVER;
    if (discipline_applied)          return NT_STRATUM_1;
    return NT_STRATUM_0;
}
...
const nt_checkpoint_t* nt_get_checkpoint(uint8_t idx) {
    if (idx >= cp_count) return nullptr;
