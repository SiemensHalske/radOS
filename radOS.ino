...
static void load_config() {
    ...
    cfg.end();
    if (cfg_low_power) cfg_log_interval = 10000;
...
void setup() {
    ...
    while ((millis() - t0) < HV_RAMP_TIMEOUT) {
        hv_update();
        if (digitalRead(FB_PIN)) saw_boost = true;
        if (saw_boost && hv_ready()) break;
        Serial.print(".");
        delay(100);
    }
    Serial.println();
    ...
    Serial.print("  HV:     ");
    if (hv_ready()) { Serial.println("PASS"); st_pass++; }
    else Serial.println("WARN — FB not ready");
    ...
    Serial.print("  SOURCE: ");
    {
        ...
        if (got > 0) {
            Serial.print("PASS  "); Serial.print(got); Serial.println(" pulses/2s");
            st_pass++;
        } else {
    ...
    Serial.print("  NVS:    ");
    { Preferences p; p.begin("selftest", true); p.end(); Serial.println("PASS"); st_pass++; }

    Serial.print("  TRNG:   ");
    if (trng_health_ok()) { Serial.println("PASS"); st_pass++; }
    else Serial.println("FAIL — health test alarm");

    Serial.print("[SELF-TEST] "); Serial.print(st_pass); Serial.println("/5 passed");
...
static void handle_command(String &cmd) {
    ...
    // Step 17: Continuous hex dump
        ...
        Serial.print("Time: "); Serial.println(rtc_timestamp());
        Serial.print("CPM: "); Serial.print(pulse_get_cpm());
        Serial.print(" | Total: "); Serial.println(pulse_get_total());
        Serial.print("HV: "); Serial.println(hv_read_mv() ? "BOOST" : "OK");
        Serial.print("Entropy: "); Serial.print(trng_available());
        Serial.print("/"); Serial.print(TRNG_POOL_BITS);
        Serial.print(" bits @ "); Serial.print(trng_rate(), 1);
        Serial.println(" bits/s");
        Serial.print("Served: "); Serial.print(trng_total_bytes());
        Serial.print(" bytes | Uptime: ");
        Serial.print(millis() / 1000);
        Serial.println("s");
        const char* strata[] = {"UNCAL", "CAL", "S0", "S1", "HOLD"};
        Serial.print("Stratum: "); Serial.print(strata[nt_get_stratum()]);
        Serial.print(" | Trust: "); Serial.print(nt_get_trust());
        Serial.print("% | Drift: "); Serial.print(nt_get_drift_ppm(), 2);
        Serial.print("ppm | Aging: "); Serial.println(rtc_get_aging());
        if (nt_is_starved()) Serial.println("ALARM: Count starvation!");
        if (nt_source_anomaly()) Serial.println("ALARM: Source anomaly!");
        if (!trng_health_ok()) Serial.println("ALARM: TRNG health test failure!");
        if (!trng_selftest_ok()) Serial.println("ALARM: TRNG self-test failure!");
    }
    else if (cmd == "TEST") {
        ...
        if (trng_read(test_buf, 32)) {
            Serial.print("HEX: ");
            for (int i = 0; i < 32; i++) {
                if (test_buf[i] < 0x10) Serial.print('0');
    ...
    // Step 28/37: Nuclear time display with trust + Kalman fused data
        ...
        Serial.print("{\"rtc\":\""); Serial.print(rtc_timestamp());
        Serial.print("\",\"temp\":"); Serial.print(rtc_temperature(), 1);
        nt_time_t nt = nt_get_time();
        if (nt.valid) {
            Serial.print(",\"nuclear_elapsed\":"); Serial.print(nt.elapsed_sec, 1);
            Serial.print(",\"uncertainty\":"); Serial.print(nt.uncertainty_sec, 1);
            Serial.print(",\"delta_ms\":"); Serial.print(nt.delta_ms);
        }
        nt_fused_t fused = nt_get_fused();
        if (fused.uncertainty_sec > 0) {
            Serial.print(",\"fused_offset\":"); Serial.print(fused.offset_sec, 3);
            Serial.print(",\"fused_uncert\":"); Serial.print(fused.uncertainty_sec, 3);
            Serial.print(",\"drift_rate\":"); Serial.print(fused.drift_rate, 6);
        }
        Serial.print(",\"lambda\":"); Serial.print(nt_get_lambda(), 4);
        Serial.print(",\"drift_ppm\":"); Serial.print(nt_get_drift_ppm(), 2);
        Serial.print(",\"trust\":"); Serial.print(nt_get_trust());
        const char* strata[] = {"UNCAL", "CAL", "S0", "S1", "HOLD"};
        Serial.print(",\"stratum\":\""); Serial.print(strata[nt_get_stratum()]);
        Serial.print("\",\"aging\":"); Serial.print(rtc_get_aging());
        if (nt_source_anomaly()) Serial.print(",\"anomaly\":true");
        if (nt_is_starved()) Serial.print(",\"starved\":true");
        Serial.println("}");
    }
    ...
    // Step 43: Checkpoint history
        ...
        if (n == 0) {
            ...
            Serial.print(n); Serial.println(" checkpoints:");
            for (uint8_t i = 0; i < n; i++) {
                const nt_checkpoint_t* cp = nt_get_checkpoint(i);
                if (!cp) continue;
                Serial.print("  #"); Serial.print(i);
                Serial.print(" epoch="); Serial.print(cp->epoch);
                Serial.print(" cnt="); Serial.print(cp->counts);
                Serial.print(" lam="); Serial.print(cp->lambda, 4);
                Serial.print(" chi2="); Serial.print(cp->chi2, 2);
                Serial.print(cp->chi2_pass ? " PASS" : " FAIL");
                Serial.print(" T="); Serial.print(cp->temp_c, 1);
                Serial.println("C");
            }
        }
    }
    // Step 35: Event log viewer
        ...
        if (n == 0) {
            ...
            Serial.print(n); Serial.println(" events:");
            char buf[48];
            for (uint8_t i = 0; i < n; i++) {
    ...
    // Step 40: SET <key> <value> — configure parameters
        ...
        if (key == "cpm_to_usv") {
            ...
            Serial.print("cpm_to_usv = "); Serial.println(cfg_cpm_to_usv, 6);
        } else if (key == "log_interval") {
            cfg_log_interval = val.toInt();
            cfg.putUInt("logms", cfg_log_interval);
            Serial.print("log_interval = "); Serial.print(cfg_log_interval); Serial.println(" ms");
        } else if (key == "neo_brightness") {
            cfg_neo_brightness = val.toInt();
            cfg.putUChar("neobri", cfg_neo_brightness);
            pulse_set_neo_brightness(cfg_neo_brightness);
            Serial.print("neo_brightness = "); Serial.println(cfg_neo_brightness);
        } else if (key == "power") {
            if (val == "low") {
                cfg_low_power = true;
    ...
    // Step 41: GET [key] — read configuration
        ...
        if (key.length() == 0) {
            ...
            Serial.print("log_interval   = "); Serial.print(cfg_log_interval); Serial.println(" ms");
            Serial.print("neo_brightness = "); Serial.println(cfg_neo_brightness);
            Serial.print("power          = "); Serial.println(cfg_low_power ? "low" : "normal");
            Serial.print("dead_time      = "); Serial.print(DEAD_TIME_US); Serial.println(" us (compile-time)");
...
void loop() {
    ...
    while (Serial.available()) {
        ...
        if (c == '\n' || c == '\r') {
            serial_cmd.trim();
            if (serial_cmd.length() > 0) handle_command(serial_cmd);
            serial_cmd = "";
        } else {
            serial_cmd += c;
        }
    ...
    if (log_enabled && !dump_mode && (now - last_log) >= cfg_log_interval) {
        ...
        if (cps_mode) {
            ...
            Serial.print(" T"); Serial.print(nt_get_trust());
            if (nt_source_anomaly()) Serial.print(" !ANOM");
            if (nt_is_starved()) Serial.print(" !STARV");
            if (!trng_health_ok()) Serial.print(" !HLTH");
            Serial.println();
        }
    }
}